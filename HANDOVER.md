...
## 最終更新: 2026-02-28
## コミット: c235706 `fix: 請求書モーダルのフィルタを社内担当者から顧客担当者に変更`
...
## バグ修正: 請求書モーダルのフィルタを社内担当者から顧客担当者に変更

**背景**: 請求書モーダルの絞り込みフィルタで「担当者名」として `personInCharge`（社内担当者）を使用していたが、請求書は顧客向けのため `clientContact`（顧客担当者）で絞り込むべきだった。

**変更箇所**: `src/App.jsx`

| 変更 | 行目安 | 内容 |
|------|--------|------|
| State初期値 | ~1901 | `invoiceFilters` の `personInCharge` → `clientContact` |
| フィルタ判定 | ~3773 | `getFilteredBillingData` 内の絞り込み条件を `u.clientContact` に変更 |
| フィルタリセット | ~4830, ~8784 | 期間変更・顧客変更時のリセット値を `clientContact: ""` に変更 |
| プルダウン選択肢 | ~8748 | 候補リストの参照を `u.clientContact` に変更 |
| ラベル表示 | ~8802 | 「担当者名」→「顧客担当者」に変更 |
| プルダウン値/onChange | ~8805-8808 | `invoiceFilters.personInCharge` → `invoiceFilters.clientContact` |
