...
## 最新の作業状況 (2026-09-11) — 出庫リストから「戻す」機能 (誤出庫の復元)

ユーザー報告: 荷物の出庫予定日を 9/10 にするつもりが誤って 9/1 を入れてしまい、`useEffect`(App.jsx:2131) の自動出庫処理で shippedUnits に移動→倉庫から消えた。UI から戻す手段が無かった。

### 追加

**1. `unshipUnit(shippedUnit, newDepartureDate)`** (src/App.jsx:2161, WarehouseView 内)
- shippedUnits から除去→units に戻す
- 元位置(`_originalLoc`)復元を試行、置けなければ `{ kind: "unplaced" }`
  - `floor`: `canPlaceOnFloor` で衝突判定
  - `shelf`/`rack`: 該当 shelf/rack が現在レイアウトに存在するかだけ確認 (中身衝突は許容)
- `stackZ=0`, `status="in_stock"`, `departureDate=newDepartureDate||null` にリセット
- editHistory に「出庫リストから復元 (元位置/未配置 / 新出庫予定: YYYY-MM-DD)」追記
- 過去日でも渡せる (呼出側で警告済み前提) — 但し `pastDate: true` を返して呼出側にヒント
- 戻り値: `{ ok, placedTo: "original"|"unplaced", pastDate } | { ok: false, reason }`

**2. `ShippedListModal` 拡張** (src/App.jsx:1522〜)
- `onRestore` prop 追加、渡された時のみ右端に「操作」列が出る
- 各行に橙「戻す」ボタン → 押下でその行が黄背景化しインライン日付ピッカー+緑「確定」+灰「取消」
- 日付初期値=今日、`min` 制約なし
- 確定時に日付が過去なら `window.confirm` で警告 (「翌ロードで再出庫される」旨) してから `onRestore` を呼ぶ
- 結果 `placedTo==="unplaced"` の時は追加で alert (元位置に置けなかった旨)
- 未使用フィールド `_originalLoc/_originalWhId/_originalWhName/shippedAt` は destructure で除去

### 今回のユーザーのケースへの対処手順

1. 該当倉庫を開く → ヘッダー右端「出庫リスト」ボタンをクリック
2. 誤出庫された荷物を検索 (顧客名 or 出庫日 9/1 で絞り込み)
3. 行の「戻す」ボタン → 日付欄に 9/10 を入力 → 確定
4. 9/10 は本日(9/11)より過去のため確認ダイアログが出る → 都合により (a)そのまま OK → 復元するが翌回また出庫される (b)キャンセルして 9/12 以降を入れる、のいずれか
...
- 棚グループ化を「1枚の仮想大棚として配置判定する」機能 (前々回積み残し)
- 復元時に shelf/rack 内の中身衝突判定 (今回は簡略化で shelf/rack 存在チェックのみ)

---

## 前回の作業状況 (2026-08-06) — 数値入力ドラフト化・リサイズ精度・境界制約
